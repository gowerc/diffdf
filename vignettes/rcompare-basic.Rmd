...
A key feature of `dfdiff` that enables easier diagnostics is the ability to specify which variables form a unique row i.e. which rows should be compared against each other based upon a key. By default if no key is specified `dfdiff` will use the rownumber as the key however in general this isn't recommended as it means two identical datasets simply sorted differently can lead to incomprehensible error messages as every observation is flagged as different.  In `dfdiff` keys can be specified as character vectors using the `keys` argument.
