...
As you would expect no differences are found. We now look to introduce various types differences into the data in order to show how `diffdf` highlights them.  Note that for the purposes of this vignette we have used the `warnings=FALSE` argument to warnings from being raised; it is recommended however that this option is not used in production code as it may mask problems. 

Note also how options are set in `diffdf_options()` which allows you to specify settings you want before doing multiple comparisons 
...
diffdf( test_data , test_data2 , keys = c("GROUP1" , "GROUP2"))
```
...
```{r, eval=FALSE}
diffdf_options(file = "reports/diffdf.log")
result <- diffdf( 
    test_data , 
    test_data2 , 
...
Sometimes it can be useful to use the comparison result to fuel further checks or programming logic. To assist with this `diffdf` offers two pieces of functionality namely the `warnings` argument (which has already been shown) and the `diffdf_has_issues()` helper function which simply returns TRUE if differences have been found else FALSE. 
...
diffdf(dsin1  , dsin2)
diffdf_options( tolerance = 0.001)
...
By default, the function will note a difference between integer and double columns, and factor and character columns. It can be useful in some contexts to prevent this from occuring. We can do so with the `strict_numeric = FALSE` and `strict_factor = FALSE` arguments. Note how we use `diffdf_options_reset()` to set options to default before adding more.
...
As we have seen, the options controlling how `diffdf` behaves can be determined via the function `diffdf_options()`. However, at the point of use, we can also specifiy options, either by passing them directly as a named argument, or via the `options` argument.
...
diffdf_options(strict_numeric = FALSE)
diffdf(dsin1  , dsin2 )
diffdf(dsin1  , dsin2, strict_numeric = TRUE )
diffdf(dsin1  , dsin2, options = diffdf_options(strict_numeric = TRUE) )
diffdf(dsin1  , dsin2 )
```

Note that `diffdf_options` will globally update an option, as can be seen in the final example.
