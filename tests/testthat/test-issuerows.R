...
test_that( "Equal datasets return no rows", {
    ...
    diff <- dfdiff( x[[1]] , x[[2]] , suppress_warnings = TRUE)
    dat <- dfdiff_issuerows(x[[1]] , diff)
...
test_that( "Checking no rows returned if only issue is missing cols" ,{
    ...
    diff <- dfdiff( x1 , x2 , suppress_warnings = TRUE)
    dat1 <- dfdiff_issuerows(x1 , diff)
    dat2 <- dfdiff_issuerows(x2 , diff)
...
test_that( "can handle multple errors including key vars",{
    ...
    diff <- dfdiff( x1 , x2 , c("ID" , "GROUP1") , suppress_warnings = TRUE)
    dat1 <- dfdiff_issuerows(x1 , diff)
    dat2 <- dfdiff_issuerows(x2 , diff)
    ...
    dat1 <- dfdiff_issuerows(x1 , diff, var = "CONTINUOUS")
    dat2 <- dfdiff_issuerows(x2 , diff, var = "CONTINUOUS")
    expect_equal(  nrow(dat1) , 3)
    expect_equal(  nrow(dat2) , 3)
    expect_equal(  ncol(dat1) , ncol(x1))
    expect_equal(  ncol(dat2) , ncol(x2))
    ...
    ### No duplicate rows
    dat1 <- dfdiff_issuerows(x1 , diff, var = c("CONTINUOUS", "CATEGORICAL"))
    dat2 <- dfdiff_issuerows(x2 , diff, var = c("CONTINUOUS", "CATEGORICAL"))
...
test_that( "Correct columns are returned where both values and columns have issues",{
    ...
    diff <- dfdiff( x1 , x2 , "ID" , suppress_warnings = TRUE)
    dat1 <- dfdiff_issuerows(x1 , diff)
    dat2 <- dfdiff_issuerows(x2 , diff)
    expect_equal(  nrow(dat1) , 3)
    expect_equal(  nrow(dat2) , 3)
