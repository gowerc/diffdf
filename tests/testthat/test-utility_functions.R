...
test_that( "dfcompare_has_issues works as expected",{
    ...
    for ( i in 1:length(list_of_comparisons)){
        ...
        if ( i %in% c(1,2)) {
            expect_false(dfcompare_has_issues(x))
        } else {
            expect_true( dfcompare_has_issues(x))
        }
    ...
    expect_true(
        dfcompare_has_issues(
            dfcompare( comp[[1]] ,  comp[[2]] , keys = c("ID" , "GROUP1"), suppress_warnings = T)  
        )
    )
    
    expect_false(
        dfcompare_has_issues(
            dfcompare( TDAT ,  TDAT , keys = c("ID" , "GROUP1"), suppress_warnings = T)  
        )
    )
