...
numdiffcheck <- function(compdat, target, value) {
    ...
    diffdf_ob <- diffdf(TDAT, compdat, suppress_warnings = TRUE)$NumDiff
...
test_that("Attribute differnce size is correct!", {
    expect_equal(
        diffdf(TDAT, TDAT_FACTCHANGE, suppress_warnings = TRUE)$AttribDiffs %>% nrow(),
        1
    )

    expect_equal(
        diffdf(TDAT, TDAT_ATTEXT, suppress_warnings = TRUE)$AttribDiffs %>% nrow(),
        1
    )

    expect_equal(
        diffdf(TDAT, TDAT_ATTEXT2, suppress_warnings = TRUE)$AttribDiffs %>% nrow(),
        2
    )

    expect_equal(
        diffdf(TDAT_ATTEXT, TDAT_ATTEXT2, suppress_warnings = TRUE)$AttribDiffs %>% nrow(),
        2
    )

    expect_equal(
        diffdf(TDAT, TDAT_LABEXT, suppress_warnings = TRUE)$AttribDiffs %>% nrow(),
        2
    )

    expect_equal(
        diffdf(TDAT, TDAT_LABEXT2, suppress_warnings = TRUE)$AttribDiffs %>% nrow(),
        1
    )
